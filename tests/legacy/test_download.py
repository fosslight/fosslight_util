...
def main():
    success, msg, _, _, _ = cli_download_and_extract("https://github.com/LGE-OSS/example",
                                                  "test_result/download/example",
                                                  "test_result/download_log/example")
    if not success:
        raise Exception(f"Download failed with git:{msg}")
    success, msg, _, _, _ = cli_download_and_extract("https://pypi.org/project/filelock/3.4.1",
